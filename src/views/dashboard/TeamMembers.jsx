...
const TeamMembers = () => {
  const members = [
    ...
    { name: 'Rahul Sharma', role: 'Employee', image: '', status: 'Absent' }, // If no image, you can show initials
  ];

  return (
    <section className={styles.teamMembers}>
      <div className={`px-3 shadow-sm rounded-2 d-flex flex-column gap-2 justify-content-between pb-3 pt-2`} style={{ height: '18.5rem', width: '430px', overflow: 'hidden', color: 'black', background: 'rgb(245, 245, 246)', border: '1px solid rgba(223, 220, 220, 0.95)' }}>
        ...
        <div className="p-1 px-3 rounded-3" style={{ height: '16rem', overflow: 'auto', background: 'rgba(237, 237, 241, 0.957)' }}>
          {members.map((member, index) => (
            <div key={index} className="d-flex align-items-center justify-content-between my-2">
              <div className="d-flex align-items-center gap-3">
                <div style={{ height: '34x', width: '30px', borderRadius: '50%', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgb(204, 204, 204)', fontSize: '14px', fontWeight: 'bold', color: 'rgb(255, 255, 255)' }}>
                  {member.image ? (
